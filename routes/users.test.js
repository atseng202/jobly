...
describe("GET /users/:username", function () {
  ...
  test("works for users own info", async function () {
    const resp = await request(app)
        .get(`/users/u1`)
  ...
  test("logged in user can't view other user", async function () {
    const resp = await request(app)
      .get(`/users/u2`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
...
describe("PATCH /users/:username", () => {
  ...
  test("works: own user set new password", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
          password: "new-password",
  ...
  test("can't set new password of other user", async function () {
    const resp = await request(app)
      .patch(`/users/u2`)
      .send({
        password: "new-password",
